...
def writing_scv(rec: list, fl: str, idr=0):
    if check_file(fl):
        with open(fl, 'a', encoding='utf-8') as f1:
            ...
            lg.logger_history("Запись новой заметки",
                              f"{rec[0]} {rec[1]} {rec[3]}", "удачно")
    else:
        print(f"файл {fl} не найден!")
        lg.logger_history("Новая заметка не сохранен",
                          f"{rec[0]} {rec[1]} {rec[3]}", "Файл не найден")
            ...
            with open('notes.csv', 'w', encoding='utf-8') as fw:
                for line in lines:
                    ...
                    if st != ids:
                        ...
                        lg.logger_history("Удаление записи", f"id={st}",
                                          "Успешно!")
                        delrec = st
                return delrec
    except:
